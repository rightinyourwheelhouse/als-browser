...
const Tutorial = ({ setTutorial }) => {
	...
	let currentContent = content[step - 1];

	return (
		<div className="absolute left-0 right-0 top-0 bottom-0 z-[999] m-auto flex h-[48rem] w-[35rem] flex-col rounded-xl bg-skin-dark p-5 pb-7">
			<div className="flex flex-grow flex-col">
				...
				<div className="mb-2 h-[17rem]">
					<img src={`${currentContent.imgLink}`} className="mx-auto object-cover" />
				</div>
				<h2 className={`${currentContent.title ? 'block' : 'hidden'} mb-5 text-xl font-bold text-[#cbd5e1]`}>
					{currentContent.title}
				</h2>
				{currentContent.text.map((par, index) => {
					return (
